...
export default async function PortalInfoPage({ searchParams }: PageProps) {
  ...
  // Tant que l'equipe n'a pas tranche, le formulaire montre ce que le client a
  // demande. Si la demande est refusee, elle n'est plus en attente et la fiche
  // reprend naturellement ses valeurs.
  const shown = {
    legalName: info.pendingUpdate?.legalName ?? info.legalName,
    siren: info.pendingUpdate?.siren ?? info.siren,
    siret: info.pendingUpdate?.siret ?? info.siret,
    billingAddress: info.pendingUpdate?.billingAddress ?? info.billingAddress,
    websiteUrl: info.pendingUpdate?.websiteUrl ?? info.websiteUrl,
    pending: info.pendingUpdate !== null,
  };
        ...
        <PortalCard>
          ...
          <form action={`${base}/informations/entreprise`} method="post" className="mt-4 grid gap-3">
            <label className="grid gap-1.5 text-sm font-medium text-zinc-800">
              {s.legalName}
              <input type="text" name="legal_name" defaultValue={shown.legalName ?? ''} maxLength={200} className={inputClass} />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1.5 text-sm font-medium text-zinc-800">
                {s.siren}
                <input type="text" name="siren" defaultValue={shown.siren ?? ''} maxLength={20} className={inputClass} />
              </label>
              <label className="grid gap-1.5 text-sm font-medium text-zinc-800">
                {s.siret}
                <input type="text" name="siret" defaultValue={shown.siret ?? ''} maxLength={20} className={inputClass} />
              </label>
            </div>
            <label className="grid gap-1.5 text-sm font-medium text-zinc-800">
              {s.billingAddress}
              <textarea
                ...
                rows={2}
                defaultValue={shown.billingAddress ?? ''}
                maxLength={400}
            ...
            <label className="grid gap-1.5 text-sm font-medium text-zinc-800">
              {s.websiteUrl}
              <input type="url" name="website_url" defaultValue={shown.websiteUrl ?? ''} maxLength={300} placeholder="https://" className={inputClass} />
            </label>
            <button
              type="submit"
