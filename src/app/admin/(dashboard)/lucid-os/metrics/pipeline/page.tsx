          ...
          <tbody>
            {pipelineDetail.map((row, i) => (
              <tr key={i} className="border-b border-zinc-100 last:border-0">
                ...
                <td className="py-3 pr-6 text-zinc-500">{row.stageLabel}</td>
                <td className="py-3 text-right tabular-nums text-zinc-900">{eur(row.valueEstimateEur)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t border-zinc-300">
              ...
              <td className="pt-3 text-right font-semibold tabular-nums text-zinc-950">{eur(kpis.openPipelineEur)}</td>
            </tr>
          </tfoot>
        </table>
      )}
