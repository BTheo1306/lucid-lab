          ...
          <tbody>
            {mrrDetail.map((row, i) => (
              <tr key={i} className="border-b border-zinc-100 last:border-0">
                ...
                <td className="py-3 pr-6 text-right tabular-nums text-zinc-900">{eur(row.monthlyValueEur)}</td>
                <td className="py-3 text-right text-zinc-500">{formatAdminDate(row.closedAt)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t border-zinc-300">
              ...
              <td className="pt-3 text-right font-semibold tabular-nums text-zinc-950">{eur(kpis.mrrEurHt)}</td>
              <td />
            </tr>
          </tfoot>
        </table>
